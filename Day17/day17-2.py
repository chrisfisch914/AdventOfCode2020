...
def parse_input(input):
    ...
    for x in range(len(input)):
        for y in range(len(input[x])):
            key = coords_to_key(x, y, 0, 0)
            lookup[key] = input[x][y] == "#"
    return lookup

def coords_to_key(x, y, z, w):
    return str(x) + ", " + str(y) + ", " + str(z) + "," + str(w)

def count_active_neighbors(lookup, x, y, z, w):
    count = 0
    for x_inc in range(-1, 2):
        for y_inc in range(-1, 2):
            for z_inc in range(-1, 2):
                for w_inc in range(-1, 2):
                    if x_inc != 0 or y_inc != 0 or z_inc != 0 or z_inc != 0 or w_inc != 0:
                        key = coords_to_key(x + x_inc, y + y_inc, z + z_inc, w + w_inc) 
    ...
    for x in range(-x_y_dist, x_y_dist + 1):
        for y in range(-x_y_dist, x_y_dist + 1):
            for z in range(-curr_round, curr_round + 1):
                for w in range(-curr_round, curr_round + 1):
                    active_neighbors = count_active_neighbors(lookup, x, y, z, w)
                    key = coords_to_key(x, y, z, w)
